import { ChangeDetectionStrategy, Component } from '@angular/core';

@Component({
  selector: 'app-need-login-page',
  imports: [],
  template: `
    <div class="text-center mt-20 lg:mt-40 fade-in">
      <h1 class="text-4xl font-extrabold tracking-tight text-gray-900 sm:text-5xl md:text-6xl">
        <span class="block xl:inline">Welcome to the</span>
        <span class="block text-indigo-600 xl:inline">Employee Portal</span>
      </h1>
      <p class="mt-3 max-w-md mx-auto text-base text-gray-500 sm:text-lg md:mt-5 md:text-xl md:max-w-3xl">
        Please authenticate via our centralized Identity Provider to access your dashboard, payslips, and candidate
        reviews.
      </p>

      <div class="mt-10 max-w-sm mx-auto sm:max-w-none sm:flex sm:justify-center lg:w-9/12">
        <button
          (click)="redirectToLogin()"
          class="w-full flex items-center justify-center px-8 py-3 border border-transparent text-base font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700 md:py-4 md:text-lg md:px-10 transition-all shadow-lg hover:shadow-xl transform hover:-translate-y-1">
          Login with SSO Provider
        </button>
      </div>
    </div>
  `,
  styles: `
    :host {
      display: block;
    }
  `,
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class NeedLoginPage {
  private readonly AUTH_APP_URL = 'http://localhost:4201';
  private readonly THIS_APP_URL = 'http://localhost:4200';

  redirectToLogin() {
    const statePayload = {
      appName: 'Workday Clone (Angular)',
      traceId: `sp-${Math.floor(Math.random() * 10000)}`,
      redirectUrl: this.THIS_APP_URL,
    };

    const encodedState = btoa(JSON.stringify(statePayload));
    window.location.href = `${this.AUTH_APP_URL}?state=${encodedState}`;
  }
}
